...
func TestLogAutoScroll(t *testing.T) {
	...
	assert.Equal(t, "Autoscroll:Off     ColumnLock:Off     FullScreen:Off     Timestamps:Off     Wrap:Off     Detail:Compact", v.Indicator().GetText(true))
}
...
func TestLogTimestamp(t *testing.T) {
	...
	l.Logs().Clear()
	ll := make([][]byte, ii.Len())
	ii.Lines(0, true, dao.LogDetailRaw, ll)
	l.Flush(ll)
