...
// SetDetailLevel sets the log detail level and refreshes the display.
func (l *Log) SetDetailLevel(level int) {
	l.logOptions.DetailLevel = level
	l.Refresh()
}
...
func (l *Log) Refresh() {
	...
	l.lines.Render(0, l.logOptions.ShowTimestamp, l.logOptions.DetailLevel, ll)
	l.fireLogChanged(ll)
}
...
func (l *Log) applyFilter(index int, q string) ([][]byte, error) {
	...
	matches, indices, err := l.lines.Filter(index, q, l.logOptions.ShowTimestamp, l.logOptions.DetailLevel)
	if err != nil {
		return nil, err
	}
	...
	if matches == nil {
		ll := make([][]byte, l.lines.Len())
		l.lines.Render(index, l.logOptions.ShowTimestamp, l.logOptions.DetailLevel, ll)
		return ll, nil
	}
	// Blank filter
	if len(matches) == 0 {
	...
	ll := make([][]byte, l.lines.Len())
	l.lines.Lines(index, l.logOptions.ShowTimestamp, l.logOptions.DetailLevel, ll)
