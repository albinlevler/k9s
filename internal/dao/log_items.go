...
func (l *LogItems) Lines(index int, showTime bool, detailLevel int, ll [][]byte) {
	l.mx.Lock()
	defer l.mx.Unlock()

	for i, item := range l.items[index:] {
		bb := bytes.NewBuffer(make([]byte, 0, item.Size()))
		item.Render(l.podColorFor(item.ID()), showTime, detailLevel, bb)
		ll[i] = bb.Bytes()
	}
}

// StrLines returns a collection of log lines.
func (l *LogItems) StrLines(index int, showTime bool, detailLevel int) []string {
	l.mx.Lock()
	defer l.mx.Unlock()
...
func (l *LogItems) Render(index int, showTime bool, detailLevel int, ll [][]byte) {
	for i, item := range l.items[index:] {
		bb := bytes.NewBuffer(make([]byte, 0, item.Size()))
		item.Render(l.podColorFor(item.ID()), showTime, detailLevel, bb)
		ll[i] = bb.Bytes()
...
func (l *LogItems) Filter(index int, q string, showTime bool, detailLevel int) (matches []int, indices [][]int, err error) {
	if q == "" {
		return
	}
	if f, ok := internal.IsFuzzySelector(q); ok {
		matches, indices = l.fuzzyFilter(index, f, showTime, detailLevel)
		return
	}
	matches, indices, err = l.filterLogs(index, q, showTime, detailLevel)
	if err != nil {
		return
	}
...
func (l *LogItems) fuzzyFilter(index int, q string, showTime bool, detailLevel int) (matches []int, indices [][]int) {
	q = strings.TrimSpace(q)
	matches, indices = make([]int, 0, len(l.items)), make([][]int, 0, len(l.items))
	mm := fuzzy.Find(q, l.StrLines(index, showTime, detailLevel))
	for _, m := range mm {
...
func (l *LogItems) filterLogs(index int, q string, showTime bool, detailLevel int) (matches []int, indices [][]int, err error) {
	var invert bool
	...
	l.Lines(index, showTime, detailLevel, ll)
	for i, line := range ll {
